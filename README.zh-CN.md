...
> 给 Claude Code 终端加一行**实时用量**的非侵入式状态行 —— 显示当前模型、上下文窗口占用、Claude.ai 的 5 小时 / 7 天（周）额度、本会话花费和改动行数，就在输入框正上方。纯标准库 Python，通过官方 `statusLine` 扩展点接入：不打补丁、升级不丢。每一列可开关、可重排。
>
> 关键词：Claude Code、Claude.ai 用量、速率限制、5 小时限制、周用量、Max 套餐、上下文窗口、token 用量、花费统计、终端状态行。
